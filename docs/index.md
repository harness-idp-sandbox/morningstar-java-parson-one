...
# run app
java -jar target/java-parson-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-parson-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-parson-one -n dev

# get external endpoint
kubectl get svc java-parson-one-svc -n dev
```
...
```bash
kubectl port-forward svc/java-parson-one-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-parson-one -n dev
kubectl rollout status deploy/java-parson-one -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javaparsonone/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javaparsonone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
